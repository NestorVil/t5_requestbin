...
async function startApp() {
  ...
  const pool = new Pool({
    ...
    ssl: {
      ...
      ca: fs
        .readFileSync(
          "/opt/capstone-bin/certs/global-bundle.pem"
        )
        .toString(),
    },
  });
  ...
  const requestHandler = async (req, res) => {
    ...
    const result = await pool.query(
      `INSERT INTO http_requests
       (basket_id, method, headers, body, path)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [
        basket.id,
  ...
  function deleteExpiredBasketsJob() {
    cron.schedule("*/20 * * * * *", async () => {
      try {
        ...
        console.log(
          `Deleted ${result.rowCount} expired basket(s)`
        );
      } catch (error) {
        console.error(
          "Failed to delete expired baskets:",
          error
        );
      }
    });
  ...
  const getBasket = async (name) => {
    const basket = await pool.query(
      "SELECT * FROM baskets WHERE name = $1",
      [name]
    );
  ...
  async function requireBasketToken(req, res, next) {
    ...
    if (!ok) {
      return res.status(403).json({
        message: "Invalid or missing basket token",
      });
    }
